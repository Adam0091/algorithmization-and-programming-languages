Surfaces.prototype.ellipsparaboloid = (point = new Point(), size = 2, a = 1, b = 2, c = 1, count = 20, color = "#FFFFFF") => {
    ...
    for(let i = 0; i < 20; i++) {
        ...
        for(let j = 0; j < 10; j++){
            if(i % 2 === 0) {
                if ((j + 1) % 3 === 0 || j % 3 === 0) {
                        poly[i * 20 + j].color = {r: 0, g: 0, b: 0}; 
                }
            } else {
                if((j - 1) % 3 === 0) {
                    poly[i * 20 + j].color = {r: 0, g: 300, b: 0};
                }
            }
